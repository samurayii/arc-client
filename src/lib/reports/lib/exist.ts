...
export async function exist (config: IAppConfig, logger: ILogger): Promise<void> {
    ...
    try {
        ...
        const report_url = `${config.keys.url}/v1/project/${config.keys.project_name}/report/${config.keys.report_name}/exist`;
        ...
        if (report_body.data === true) {
            logger.log(chalk.green("Report exist"));
        } else {
            logger.log(chalk.red("Report not exist"));
        }
